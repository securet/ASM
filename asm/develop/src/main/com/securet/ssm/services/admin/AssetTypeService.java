...
@Service
@Controller
public class AssetTypeService extends SecureTService {
	...
	public static List<String> getFieldsToExcludeInDisplay(){
		...
		if(excludeInDisplay.isEmpty()){
			excludeInDisplay = new ArrayList<String>();
			excludeInDisplay.add("assetTypeId");
	...
	@RequestMapping(value="/admin/saveAssetType",method=RequestMethod.POST)
	@Transactional
	public String saveAssetType(@Valid @ModelAttribute("formObject") AssetType formObject,BindingResult result,Model model){
		boolean createNew = (formObject.getAssetTypeId()==0);//default is 0..
