...
@Service
@Controller
public class IssueTypeService extends SecureTService {
	...
	public static List<String> getFieldsToExcludeInDisplay(){
		...
		if(excludeInDisplay.isEmpty()){
			excludeInDisplay = new ArrayList<String>();
			excludeInDisplay.add("issueTypeId");
	...
	@RequestMapping(value="/admin/saveIssueType",method=RequestMethod.POST)
	@Transactional
	public String saveIssueType(@Valid @ModelAttribute("formObject") IssueType formObject,BindingResult result,Model model){
		boolean createNew = (formObject.getIssueTypeId()==0);//default is 0..
