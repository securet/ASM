...
public class SQLUserLogin extends com.mysema.query.sql.RelationalPathBase<SQLUserLogin> {
    ...
    public void addMetadata() {
        ...
        addMetadata(disabledTimestamp, ColumnMetadata.named("disabledTimestamp").withIndex(4).ofType(Types.TIMESTAMP).withSize(19));
        addMetadata(enabled, ColumnMetadata.named("enabled").withIndex(5).ofType(Types.BIT).withSize(1).notNull());
        addMetadata(lastLoginTimestamp, ColumnMetadata.named("lastLoginTimestamp").withIndex(6).ofType(Types.TIMESTAMP).withSize(19));
